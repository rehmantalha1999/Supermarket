...
{
    global $db;
    $stmt = $db->prepare("INSERT INTO consumer(email, name, password, city, district, address, verificationNumber) VALUES (?,?,?,?,?,?,?)");
...
{
    global $db;
    $stmt = $db->prepare("update consumer set isVerified=1 where email=?");
    $stmt->execute([$email]);
}
